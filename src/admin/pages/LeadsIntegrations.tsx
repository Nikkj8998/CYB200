...
export const LeadsIntegrations = () => {
  ...
  const processSpreadsheet = async (spreadsheet: SpreadsheetConfig) => {
    ...
    try {
      ...
      if (successCount > 0) toast.success(`Spreadsheet "${spreadsheet.name}": Imported ${successCount} leads`);
      if (failedCount > 0) toast.warning(`Spreadsheet "${spreadsheet.name}": Failed to import ${failedCount} leads`);
      if (duplicateCount > 0) toast.info(`Spreadsheet "${spreadsheet.name}": Skipped ${duplicateCount} duplicate leads`);
    } catch (error: any) {
    ...
    try {
      ...
      for (const spreadsheet of activeSpreadsheets) {
        try {
          ...
          if (successCount > 0) toast.success(`"${spreadsheet.name}": Imported ${successCount} leads`);
          if (duplicateCount > 0) toast.info(`"${spreadsheet.name}": Skipped ${duplicateCount} duplicate leads`);
          if (failedCount > 0) toast.warning(`"${spreadsheet.name}": Failed to import ${failedCount} leads`);
            ...
            {importResults && (
              <div className="border rounded-lg p-4 bg-gray-50">
                ...
                {importResults.errors.length > 0 && (
                  <div className="mt-3">
                    <h5 className="font-medium mb-1 text-sm">Errors ({importResults.errors.length}):</h5>
